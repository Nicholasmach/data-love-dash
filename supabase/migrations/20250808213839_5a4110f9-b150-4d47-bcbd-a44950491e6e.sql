-- Corrigir função execute_analytics_query para permitir funções agregadas
...
BEGIN
    ...
    -- Verificar se contém operações potencialmente perigosas (mais específicas)
    IF safe_query ~* '(DROP|DELETE|UPDATE|INSERT|ALTER|CREATE|TRUNCATE|GRANT|REVOKE)' THEN
        RAISE EXCEPTION 'Potentially harmful SQL operations are not allowed';
    END IF;
    
    -- Verificar se há tentativa de múltiplas queries (injection)
    IF safe_query ~* ';.*SELECT' THEN
